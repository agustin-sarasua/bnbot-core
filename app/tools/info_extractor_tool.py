...
class InfoExtractorChain:
    ...
    def __init__(self):
        ...
        prompt_template = PromptTemplate(
            input_variables=["time", "conversation"], 
    ...
    def __call__(self, conversation):
        ...
        info = self.chain({"time":formatted_date, "conversation": conversation})
        return self.output_parser.parse(info["booking_information"])
    

class InfoExtractorTool(BaseTool):
    name = "info_extractor"
    description = "useful for when you need to calculate the check-in, check-out and number of guests based on the user input."
